...
The plot suggests that the model I selected (`model_1`) is a clear winner because it has lowest mean RMSE value and the overall distribution of RMSE values is relatively low. The model using main effects only (`model_2`) is the worst because its RMSE values are really high. The three-way interaction model (`model_3`) is better than the main effect model because it has a rather lower distribution of RMSE values.
